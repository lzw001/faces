...
class FaceLandmarksTransformer(object):
	...
	def transform(
		...
		for path_to_class in path_to_classes:
			...
			classes.extend([path_to_class for _ in len(images_in_directory)])
		classes = np.array(classes)
		landmarks = np.zeros(shape=(len(images, 1/2 * (68 * 68 - 68))))
		for i, image in enumerate(images):
			landmarks[i, :] = self.face_landmarks_distances(image)
		return landmarks, classes

	def face_landmarks_distances(image: str) -> np.ndarray:
